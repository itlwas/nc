...
void render_scroll(void) {
    ...
    size_t margin = (editor.rows <= VSCROLL_MARGIN * 2) ? 0 : VSCROLL_MARGIN;
    ...
    if (editor.file.cursor.rx < editor.window.x + hmargin) {
        ...
        editor.window.x = editor.file.cursor.rx - text_width + hmargin + 1;
    }
    ...
    for (size_t y = 0; y < editor.rows; ++y) {
        ...
        if (!line) {
            if (y == editor.rows / 3 &&
                ...
                char msg[32];
                snprintf(msg, sizeof(msg), "nc ~ v%s", NC_VERSION);
...
static void draw_line(size_t row, const unsigned char *line, size_t len) {
    ...
    size_t prev_len = editor.screen_lens[row];
    if (prev_len == len && memcmp(editor.screen_lines[row], line, len) == 0) {
        return;
    }
    size_t common_len = 0;
    while (common_len < prev_len &&
           common_len < len &&
    ...
    size_t common_width = length_to_width((unsigned char *)editor.screen_lines[row], common_len);
    term_set_cursor(common_width, row);
    if (common_len < len) {
        term_write(line + common_len, len - common_len);
    }
    size_t new_width = common_width + length_to_width(line + common_len, len - common_len);
    size_t old_width = length_to_width((unsigned char *)editor.screen_lines[row], prev_len);
